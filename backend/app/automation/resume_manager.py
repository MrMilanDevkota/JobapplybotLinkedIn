import json
import os
from app.config.config import Config # Import Config to get file paths and some initial data

class ResumeManager:
    def __init__(self):
        self.config = Config()
        self.RESUME_DATA_FILE = self.config.RESUME_DATA_FILE
        self.LINKEDIN_EMAIL = self.config.LINKEDIN_EMAIL
        self.PHONE_NUMBER = self.config.PHONE_NUMBER
        self.USER_WEBSITE = self.config.USER_WEBSITE
        # yo sabai user le input garne kura haru hunxa form ma

    def load_resume_data(self):
        """Load resume data from JSON file or create if not exists"""
        try:
            if os.path.exists(self.RESUME_DATA_FILE):
                with open(self.RESUME_DATA_FILE, 'r') as file:
                ...
                resume_data = {
                    ...
                    "resume_path": self.config.RESUME_PATH # Add resume path from config
                }

                # Save template to file
                with open(self.RESUME_DATA_FILE, 'w') as file:
                    json.dump(resume_data, file, indent=4)

                print(f"Created template resume data file at {self.RESUME_DATA_FILE}. Please update with your information.")
                return resume_data
